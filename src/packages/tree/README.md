...
export default {
  setup() {
    ...
    const click = (item, resolve) => {
      if (!item.isLoad) { // false表示还没展开加载过
        let newData = []
        if (item.id === 'gd') {
          // 根据当前项的相关参数请求下一级
          newData = [{label: '天河区', hasChild: false}, {label: '白云区', hasChild: false}]
        }
        if (item.id === 'gx') {
          // 根据当前项的相关参数请求下一级
          newData = [{label: '桂林市', hasChild: false}, {label: '南宁市', hasChild: false}]
        }
        if (item.id === 'bj') {
          // 根据当前项的相关参数请求下一级
          newData = [{label: '朝阳区', hasChild: false}, {label: '东城区', hasChild: false}]
        }
        // 模拟请求数据
        setTimeout(() => {
