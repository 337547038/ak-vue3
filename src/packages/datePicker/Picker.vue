...
<template>
  <div class="calendar">
    <div class="calendar-head">
      ...
      <a
        v-if="state.showMonthControl && state.activePanel !== 'Year'"
        class="calendar-prev-month"
        @click="monthClick(0)"
        >‹</a
      >
      <a class="calendar-year-select" @click="state.activePanel = 'Year'"
        >{{ year }}年</a
      >
      <a
        v-if="state.showMonth"
        class="calendar-year-month"
        @click="state.activePanel = 'Month'"
        >{{ month }}月</a
      >
      <a
        v-if="state.showMonthControl && state.activePanel !== 'Year'"
    ...
    <div class="calendar-body">
      <component
        :is="state.activePanel"
        v-model="state.activeValue"
        :body-value="state.bodyValue"
        :type="type"
        @click="paneClick"
      />
    </div>
...
<script lang="ts" setup>
  ...
  const props = withDefaults(
    defineProps<{
      value: Date
      type: 'year' | 'month' | 'date' | 'datetime' // // 四种类型，年/年月/年月日/年月日时分秒
    }>(),
    {
      type: 'date'
    }
  ...
  if (props.type === 'year') {
    // 年或年月选择时，默认显示年
    state.activePanel = markRaw(Year)
  } else if (props.type === 'month') {
    state.activePanel = markRaw(Month)
  } else {
    state.activePanel = markRaw(Day)
  }
  const year = computed(() => {
  ...
  const yearClick = (type: number) => {
    ...
    if (state.activePanel === 'Year') {
      state.bodyValue = new Date(
        state.bodyValue.setFullYear(state.bodyValue.getFullYear() + num)
      )
    } else {
      state.activeValue = new Date(
        state.activeValue.setFullYear(state.activeValue.getFullYear() + num)
      )
    }
  }
  const monthClick = (type: number) => {
    ...
    state.activeValue = new Date(
      state.activeValue.setMonth(state.activeValue.getMonth() + num)
    )
  }
