...
<script lang="ts" setup>
  ...
  const confirmClick = (e: MouseEvent) => {
    console.log(e)
    state.visible = false
    emits('update:modelValue', colorToRgba.value)
  ...
  const showClick = (e: any) => {
    if (el.value.contains(e.target)) {
      ...
      nextTick(() => {
        state.sideBarHeight =
          el.value.querySelector('.color-slider').offsetHeight
        panel.value.calcXY()
      })
    } else {
      state.visible = false
