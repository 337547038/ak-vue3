...
<template>
  <transition :name="animation">
    ...
    >
      <div
        ref="dialogEl"
        :class="{ [prefixCls + '-dialog']: true, [state.className]: className, [prefixCls + '-dialog-isAlert']: isAlert }"
        :style="{ width: width, top: top, left: state.left, 'transition-duration': state.moveFlag ? '0s' : '' }"
        @click.stop
      >
        <a
          v-if="showClose"
        ...
        <div v-if="autoClose > 0" :class="`${prefixCls}-dialog-auto-close`">
          <span v-html="closeTips.replace('S', state.autoTime)"></span>
        </div>
        <div
        ...
        >
          ...
          <div v-if="content" :class="[prefixCls + '-dialog-text']" v-html="content"></div>
          <slot v-else></slot>
        </div>
